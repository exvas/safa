frappe.ui.form.on('Sales Invoice', {
    ...
    refresh: function(frm) {
        ...
        // Setup mobile-friendly item removal functionality
        setTimeout(() => {
            setup_mobile_item_removal(frm);
        }, 1000);
    }
...
frappe.ui.form.on('Sales Invoice Item', {
    ...
    items_add: function(frm, cdt, cdn) {
        ...
        // Add mobile-friendly item removal functionality after a short delay
        setTimeout(() => {
            setup_mobile_item_removal(frm);
        }, 500);
    }
...
// Function to setup mobile-friendly item removal
function setup_mobile_item_removal(frm) {
    // Target the items table specifically
    const items_wrapper = frm.fields_dict.items.wrapper;
    if (!items_wrapper) return;
    
    // Remove existing event listeners to prevent duplicates
    $(items_wrapper).off('click.mobile_remove touchstart.mobile_remove');
    
    // Add both click and touch event handlers for cross-platform compatibility
    $(items_wrapper).on('click.mobile_remove touchstart.mobile_remove', '.grid-remove-rows, .btn-remove', function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        console.log("Mobile item removal triggered");
        
        // Find the row to remove
        let $btn = $(this);
        let $row = $btn.closest('.grid-row');
        
        if ($row.length === 0) {
            // Try alternative selectors for different Frappe versions
            $row = $btn.closest('[data-idx]');
        }
        
        if ($row.length > 0) {
            let idx = $row.attr('data-idx');
            if (idx) {
                // Use Frappe's built-in method to remove the row
                let grid = frm.fields_dict.items.grid;
                if (grid && grid.grid_rows && grid.grid_rows[idx - 1]) {
                    grid.grid_rows[idx - 1].remove();
                    frm.refresh_field('items');
                    console.log("Item removed successfully, idx:", idx);
                }
            }
        }
    });
    
    // Add touch-friendly styling for Android Chrome
    $(items_wrapper).find('.grid-remove-rows, .btn-remove').each(function() {
        $(this).css({
            'min-width': '44px',
            'min-height': '44px',
    ...
    // Handle checkbox-based row selection and removal
    $(items_wrapper).on('change.mobile_checkbox', 'input[type="checkbox"]', function(e) {
        let $checkbox = $(this);
        let $row = $checkbox.closest('.grid-row');
        ...
        // Update remove button visibility/state
        update_remove_button_state(frm);
    });
    
    // Enhanced remove button handler for selected rows
    $(items_wrapper).on('click.mobile_remove_selected touchstart.mobile_remove_selected', '.grid-remove-rows', function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        console.log("Remove selected rows triggered");
        ...
        let selected_rows = [];
        $(items_wrapper).find('input[type="checkbox"]:checked').each(function() {
            let $row = $(this).closest('.grid-row');
            let idx = $row.attr('data-idx');
            if (idx) {
                selected_rows.push(parseInt(idx));
            }
        });
        
        if (selected_rows.length === 0) {
            frappe.msgprint(__('Please select rows to remove'));
            return;
        }
        
        // Remove rows in reverse order to maintain indices
        selected_rows.sort((a, b) => b - a);
        selected_rows.forEach(idx => {
            if (grid.grid_rows && grid.grid_rows[idx - 1]) {
                grid.grid_rows[idx - 1].remove();
            }
        });
        
        frm.refresh_field('items');
        console.log("Selected items removed successfully:", selected_rows);
    });
}

// Function to update remove button state based on selection
function update_remove_button_state(frm) {
    const items_wrapper = frm.fields_dict.items.wrapper;
    if (!items_wrapper) return;
    
    let selected_count = $(items_wrapper).find('input[type="checkbox"]:checked').length;
    let $remove_btn = $(items_wrapper).find('.grid-remove-rows');
    
    if (selected_count > 0) {
        $remove_btn.removeClass('disabled').prop('disabled', false);
        $remove_btn.attr('title', `Remove ${selected_count} selected item(s)`);
    } else {
        $remove_btn.addClass('disabled').prop('disabled', true);
        $remove_btn.attr('title', 'Select items to remove');
    }
}
...
// Global event delegation for cross-platform item removal compatibility
$(document).ready(function() {
    // Use document-level event delegation for maximum compatibility
    $(document).on('click touchstart', '[data-fieldname="items"] .grid-remove-rows, [data-fieldname="items"] .btn-remove, [data-fieldname="items"] [data-action="remove_row"]', function(e) {
        // Only handle if this is a Sales Invoice form
        if (!cur_frm || cur_frm.doctype !== 'Sales Invoice') return;
        
        e.preventDefault();
        e.stopPropagation();
        
        console.log("Global item removal handler triggered");
        
        let $btn = $(this);
        let $wrapper = $btn.closest('[data-fieldname="items"]');
        
        if ($wrapper.length === 0) return;
        
        // Handle individual row removal (x button)
        if ($btn.hasClass('btn-remove') || $btn.attr('data-action') === 'remove_row') {
            let $row = $btn.closest('.grid-row');
            if ($row.length > 0) {
                let idx = $row.attr('data-idx');
                if (idx && cur_frm.fields_dict.items.grid.grid_rows[idx - 1]) {
                    cur_frm.fields_dict.items.grid.grid_rows[idx - 1].remove();
                    cur_frm.refresh_field('items');
                    console.log("Individual item removed, idx:", idx);
                }
            }
        }
        // Handle bulk removal (remove selected button)
            ...
            selected_rows.sort((a, b) => b - a);
            selected_rows.forEach(idx => {
                if (cur_frm.fields_dict.items.grid.grid_rows[idx - 1]) {
                    cur_frm.fields_dict.items.grid.grid_rows[idx - 1].remove();
                }
            });
            
            cur_frm.refresh_field('items');
            console.log("Bulk items removed:", selected_rows);
        }
    });
    
    // Handle checkbox changes for row selection
    $(document).on('change', '[data-fieldname="items"] input[type="checkbox"]', function(e) {
        if (!cur_frm || cur_frm.doctype !== 'Sales Invoice') return;
        
        let $checkbox = $(this);
        let $row = $checkbox.closest('.grid-row');
        let $wrapper = $checkbox.closest('[data-fieldname="items"]');
        ...
        let selected_count = $wrapper.find('input[type="checkbox"]:checked').length;
        let $remove_btn = $wrapper.find('.grid-remove-rows');
